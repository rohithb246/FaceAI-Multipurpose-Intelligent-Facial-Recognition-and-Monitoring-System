...
class Database:
    ...
    def init_database(self):
        ...
        # Create feedback/contact submissions table
        cursor.execute('''
    ...
    def change_user_password(self, user_id, current_password, new_password):
        ...
        try:
            ...
            if current_hash != current_password_hash:
                conn.close()
                return False
    ...
    def add_feedback(self, name, email, subject, message):
        """Add a new feedback/contact submission"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
    ...
    def get_all_feedback(self):
        """Get all feedback submissions for admin panel"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT f.id, f.name, f.email, f.subject, f.message, 
                   f.submitted_at, f.status, f.admin_response, 
                   f.responded_at, u.full_name as responder_name
            FROM feedback f
        ...
        for feedback in feedback_data:
            feedback_list.append({
                ...
                'status': feedback[6],
                'admin_response': feedback[7],
                'responded_at': feedback[8],
                'responder_name': feedback[9]
            })
        
        return feedback_list
    
    def update_feedback_status(self, feedback_id, status, admin_response=None, responder_id=None):
        ...
        try:
            ...
            if admin_response:
                cursor.execute('''
                    UPDATE feedback 
                    SET status = ?, admin_response = ?, responded_at = ?, responded_by = ?
                    WHERE id = ?
                ''', (status, admin_response, datetime.now(), responder_id, feedback_id))
            else:
                cursor.execute('''
                    UPDATE feedback 
                    SET status = ?
    ...
    def get_feedback_by_id(self, feedback_id):
        ...
        if feedback_data:
            return {
                ...
                'email': feedback_data[2],
                'subject': feedback_data[3],
                'message': feedback_data[4],
                'submitted_at': feedback_data[5],
                'status': feedback_data[6],
                'admin_response': feedback_data[7],
                'responded_at': feedback_data[8],
                'responder_name': feedback_data[9]
            }
        return None
